...
[[nodiscard]] TLinputMessageContent MessageContentFromFile(
	...
	const auto attached = tl_vector<TLint32>(file->attachedStickers
		...
		| ranges::to<QVector>());
	if (file->to.options.price > 0) {
		return tl_inputMessagePaidMedia(
			tl_int53(file->to.options.price),
			tl_vector<TLinputPaidMedia>(1, tl_inputPaidMedia(
				(file->filetype == PreparedFileType::Photo
					? tl_inputPaidMediaTypePhoto()
					: tl_inputPaidMediaTypeVideo(
						tl_int32(seconds),
...
void SendPreparedAlbumIfReady(
	...
	if (album->items.empty()) {
		...
		return;
	} else if (action.options.price > 0) {
		const auto &first = album->items.front().content;
		const auto &data = first->c_inputMessagePaidMedia();
		auto medias = QVector<TLinputPaidMedia>();
		medias.reserve(album->items.size());
		for (const auto &item : album->items) {
			if (!item.content) {
				return;
			}
			const auto &data = item.content->c_inputMessagePaidMedia();
			medias.push_back(data.vpaid_media().v.front());
		}
		SendPreparedMessage(action, tl_inputMessagePaidMedia(
			data.vstar_count(),
			tl_vector<TLinputPaidMedia>(std::move(medias)),
			data.vcaption() ? *data.vcaption() : TLformattedText(),
			data.vshow_caption_above_media()));
		return;
	}
