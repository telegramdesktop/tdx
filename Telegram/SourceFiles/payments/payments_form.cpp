...
TLinputInvoice Form::inputInvoice() const {
	return ComputeInputInvoice(_id);
}

TLinputInvoice ComputeInputInvoice(const InvoiceId &id) {
	Expects(!v::is<InvoiceStarGift>(id.value));
	...
	} else if (const auto credits = std::get_if<InvoiceCredits>(&id.value)) {
		if (const auto userId = peerToUser(credits->giftPeerId)) {
			return tl_inputInvoiceTelegram(
				tl_telegramPaymentPurposeGiftedStars(
	...
	const auto &giftCode = v::get<InvoicePremiumGiftCode>(id.value);
	if (giftCode.creditsAmount) {
		return tl_inputInvoiceTelegram(tl_telegramPaymentPurposeStarGiveaway(
			InvoiceGiftCodeGiveawayToTL(giftCode),
			tl_string(giftCode.currency),
			tl_int53(giftCode.amount),
			tl_int32(giftCode.users),
			tl_int53(*giftCode.creditsAmount)));
	}
	const auto users = std::get_if<InvoicePremiumGiftCodeUsers>(
		&giftCode.purpose);
	if (users) {
