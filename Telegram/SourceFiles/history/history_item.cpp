...
: HistoryItem(history, {
	...
	if (!emptyReactions) {
		const auto &reactions = interaction->data().vreactions()->data();
		setReactions(
			reactions.vreactions().v,
			data.vunread_reactions().v,
			reactions.vare_tags().v,
			reactions.vcan_get_added_reactions().v);
...
void HistoryItem::setReactions(
	...
	if (changeReactions(list, areTags, canView)
		&& changeUnreadReactions(unread)) {
		_flags |= MessageFlag::HasUnreadReaction;
	}
}
...
void HistoryItem::updateReactions(
	...
	const auto changed = changeReactions(list, areTags, canView);
	if (!changed) {
		return;
	}
		...
		prepared.text = {
			(action.vis_unclaimed().v
				...
				: tr::lng_prize_about)(
					...
					lt_channel,
					_from->owner().peer(
						peerFromTdbChat(action.vboosted_chat_id()))->name()),
		};
